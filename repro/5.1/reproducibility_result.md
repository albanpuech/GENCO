...
Not re-run: Hugging Face download, conversion, `gridfm_graphkit train` / `evaluate`, or `aggregate_mlflow_metrics.py`. GENCO means and stds are those already in `results/{1.1,1.2,1.3,2.3,3.1,4.1,4.2,4.3}/metrics_summary.csv` on `genco-paper-repro-pfdelta`.

## Pipeline that was run

Working directory: `/Users/apu/repro/gridfm-graphkit` (plot scripts now live in `scripts/pfdelta/`, renamed from `examples/`). Original copies were under `/Users/apu/to_del_just_to_evaluate_contingency/gridfm-graphkit/examples/pfdelta`, with two compatibility edits: parse OPF-style `\std` cells; read `results/` from the repo root.
