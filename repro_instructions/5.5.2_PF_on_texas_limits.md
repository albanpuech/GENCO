...
The loading and voltage figures are rebuilt from the light tables, not from a metrics CSV. The published tables are `data/light/` in [gridfm/genco-pf-contingency](https://huggingface.co/gridfm/genco-pf-contingency).
...
```bash
hf download gridfm/genco-pf-contingency \
  --include "mlflow/eval/test_2/**" --include "data/**" \
  --local-dir genco-pf-contingency
```
...
python scripts/contingency/create_minimal_contingency_data.py \
  --predictions genco-pf-contingency/mlflow/eval/test_2/predictions.parquet \
  --bus-data genco-pf-contingency/data/test_2/bus_data.parquet \
  --branch-data genco-pf-contingency/data/test_2/branch_data.parquet \
  --output-dir light
```
...
python scripts/contingency/contingency_analysis.py \
  --light-dir genco-pf-contingency/data/light \
  --output-dir figures
```
