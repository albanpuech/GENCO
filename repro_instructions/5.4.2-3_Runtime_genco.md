...
PF inference timings for the [GENCO paper](https://arxiv.org/abs/2608.09921). The OPF GENCO curves use these same PF timings. There is no separate OPF GENCO benchmark. Figure 10 keeps the PowerModels OPF curves from [5.4.2-3_Runtime_powermodels.md](5.4.2-3_Runtime_powermodels.md).
...
The first 10,000 `data_index_*.pt` files per network are on Hugging Face as one uncompressed tar per grid: [`gridfm/genco-pf-runtime`](https://huggingface.co/datasets/gridfm/genco-pf-runtime).

```bash
hf download gridfm/genco-pf-runtime --repo-type dataset \
    --local-dir /path/to/pf
for t in /path/to/pf/*.tar; do tar -xf "$t" -C /path/to/pf; done
export GENCO_DATA_PATH=/path/to/pf
```

Each `<network>.tar` contains `<network>/processed/data_index_0.pt` through `data_index_9999.pt`. The dataset has IEEE 14, 30, 57, 118 and GOC 500, 2000, and 10000.
