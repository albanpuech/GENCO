...
The training grid and each `test_<k>` grid are in the same repo as raw parquet only, at `data/<split>/Texas2k_case1_2016summerpeak/raw/`. There is no `processed/` directory. `--data_path` is `genco-pf-contingency/data/<split>`.
