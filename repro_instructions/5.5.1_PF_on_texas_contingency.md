...
The training grid and each `test_<k>` grid are in the same repo as raw parquet only, at `data/<split>/Texas2k_case1_2016summerpeak/raw/`. There is no `processed/` directory. `--data_path` is `genco-pf-contingency-base/data/<split>`.

```bash
hf download gridfm/genco-pf-contingency-base --include "data/training_dataset/**" --local-dir genco-pf-contingency-base
hf download gridfm/genco-pf-contingency-base --include "data/test_1/**" --local-dir genco-pf-contingency-base
```
...
gridfm_graphkit evaluate \
  ...
  --data_path genco-pf-contingency-base/data/test_2 \
