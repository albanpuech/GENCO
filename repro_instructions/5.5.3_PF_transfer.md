...
Checkpoints, the training and eval MLflow runs, and the eval parquet are in [gridfm/genco-pf-transfer](https://huggingface.co/gridfm/genco-pf-transfer). Each checkpoint folder contains the YAML and the shell command used for that run.

## Pretraining

### Data

100 training subgrids, 10 validation subgrids, and 10 test subgrids. Download [gridfm/genco-pf-pretraining](https://huggingface.co/datasets/gridfm/genco-pf-pretraining). Training names are `train_networks.txt`. Validation and test names are `validation_networks.txt` and `test_networks.txt`.

```bash
hf download gridfm/genco-pf-pretraining --repo-type dataset --local-dir data_pretrain
mv data_pretrain/casefiles casefiles
```

Move `casefiles/` out before training. Graphkit treats every top-level directory as a grid, so `casefiles/` would enter the draw.
...
Clone [`genco-paper-repro-pretraining`](https://github.com/gridfm/gridfm-graphkit/tree/genco-paper-repro-pretraining) and train with [`HGNS_PreTrain_subnets_100.yaml`](https://github.com/gridfm/gridfm-graphkit/blob/genco-paper-repro-pretraining/examples/config/HGNS_PreTrain_subnets_100.yaml): 100 networks, seed 200, 300 epochs, batch size 32.
...
The paper checkpoint is `pretraining/best_model_state_dict.pt`. The same folder contains `HGNS_PreTrain_subnets_100.yaml` and `train.sh`. Logs are in `mlflow/pretrain/1_h_100_32`. There is no `normalizer_stats.pt`.

```bash
hf download gridfm/genco-pf-transfer --include "pretraining/**" --local-dir genco-pf-transfer
```
...
Datakit PF samples for `case118_ieee` are [gridfm/pf_small_case118_ieee](https://huggingface.co/datasets/gridfm/pf_small_case118_ieee), in the [datakit_pf_datasets](https://huggingface.co/collections/gridfm/datakit-pf-datasets) collection. `n_scenarios.txt` is 219,103. Graphkit loads `{data_path}/case118_ieee/raw/*.parquet`.
...
gridfm_graphkit finetune \
  --config scripts/pretraining_advantage/configs/case118_1000.yaml \
  --data_path data \
  --model_path genco-pf-transfer/pretraining/best_model_state_dict.pt
```
...
```bash
hf download gridfm/genco-pf-transfer --include "data/**" --local-dir genco-pf-transfer
```
...
gridfm_graphkit evaluate \
  ...
  --data_path genco-pf-transfer/data \
  --model_path genco-pf-transfer/finetune/case118_1000/best_model_state_dict.pt \
  --normalizer_stats genco-pf-transfer/finetune/case118_1000/normalizer_stats.pt \
