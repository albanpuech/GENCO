...
This covers the appendix optimality and feasibility table (`tab:opf_scaling`) for GENCO Base and Small. The runtime figure and the speedup columns are in [5.4.2-3_Runtime](5.4.2-3_Runtime.md).
...
Repeat for the other grids (`data/<network>/raw`).
...
**Checkpoints:** [gridfm/genco-opf-datakit](https://huggingface.co/gridfm/genco-opf-datakit)
...
The training runs that match the table are in the same repo under [`mlflow/train/`](https://huggingface.co/gridfm/genco-opf-datakit/tree/main/mlflow/train).
