...
This covers the residual-vs-grid-size plot (`fig:gridsize_vs_residuals_pf`). The runtime figures and speedup columns are in [5.4.2-3_Runtime](5.4.2-3_Runtime.md).
...
Repeat for the other grids (`data/case14_ieee/raw`, `data/case500_goc/raw`, …).
...
**Checkpoints:** [gridfm/genco-pf-datakit](https://huggingface.co/gridfm/genco-pf-datakit)
...
The MLflow runs are in the same repo under [`mlflow/`](https://huggingface.co/gridfm/genco-pf-datakit/tree/main/mlflow). For grids through GOC 500 the figure uses `mlflow/eval/`. For GOC 2000 and GOC 10000 it uses `mlflow/train/`.
